...
void push_through_iso(GF *A, GF *B, GF *A24,
                      ...
                      const int ell, const int *strategy, int h,
                      GF *Px, GF *Py, GF *Pz,
...
/*
      ...
      printf("\n");
*/
      rdata[r] = concat(E_R, E_RS, hrc0, hrc1, NULL);
      //printf("\nround %d data: %s\n", r, rdata[r]);
...
double iu_sign(double *time, char * eA_str, char * eB_str, char * lA_str, char * lB_str, int *strA, int lenA, int *strB, int lenB, 
  ...
  */
  ...
  for (int t=0; t<NUM_THREADS; t++) {
    ...
    Ecopy=malloc(sizeof(MC));
    E_Scopy=malloc(sizeof(MC));
    Scopy=malloc(sizeof(MP));
    PBcopy=malloc(sizeof(MP));
    QBcopy=malloc(sizeof(MP));
    phiPBcopy=malloc(sizeof(MP));
    phiQBcopy=malloc(sizeof(MP));
    init_MC(Ecopy);
    init_MC(E_Scopy);
    init_MP(Scopy);
    init_MP(PBcopy);
